...
import { Save, User as UserIcon, Users, Building2, Bell, Store, Upload, X } from "lucide-react";
...
function ProfileTab() {
  ...
  return (
    <div className="space-y-6 max-w-lg">
      ...
      <Button className="gap-2" onClick={() => toast({ title: "ကိုယ်ရေးအချက်အလက်သိမ်းပြီး" })}>
        <Save className="h-4 w-4" />ကိုယ်ရေးအချက်အလက်သိမ်းမည်
      </Button>
      <Separator />
      <div className="space-y-3">
        <h3 className="font-medium">စကားဝှက်ပြောင်းမည်</h3>
        <div className="space-y-1.5"><Label>လက်ရှိစကားဝှက်</Label><Input type="password" placeholder="••••••••" /></div>
        <div className="space-y-1.5"><Label>စကားဝှက်အသစ်</Label><Input type="password" placeholder="••••••••" /></div>
        <div className="space-y-1.5"><Label>စကားဝှက်အသစ် အတည်ပြုမည်</Label><Input type="password" placeholder="••••••••" /></div>
...
function BrandingTab() {
  ...
  const [form, setForm] = useState<Pick<Branding, "shopName" | "shopTagline" | "logoUrl">>({
    shopName: branding.shopName, shopTagline: branding.shopTagline, logoUrl: branding.logoUrl,
  });
  ...
  const shopInitials = form.shopName.split(/\s+/).map((w) => w[0]).join("").toUpperCase().slice(0, 2) || "SM";

  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 1024 * 1024) { toast({ title: "လိုဂိုဖိုင်သည် 1MB ထက် မကျော်ရ", variant: "destructive" }); return; }
  ...
  const handleSave = async () => {
    setSaving(true);
    try { await updateBranding(form); toast({ title: "ဆိုင်အချက်အလက်သိမ်းပြီး" }); }
    catch { toast({ title: "သိမ်းမသိမ်းနိုင်ပါ", variant: "destructive" }); }
  ...
  return (
    <div className="space-y-6 max-w-lg">
      <div>
        ...
        <p className="text-sm text-muted-foreground">ဆိုင်အမည်နှင့် လိုဂိုကို ပြောင်းလဲပါက sidebar နှင့် app တစ်ခုလုံးတွင် ချက်ချင်းဖေါ်ပြမည်။</p>
      </div>
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-sidebar-primary text-white font-bold text-sm overflow-hidden">
              {form.logoUrl ? <img src={form.logoUrl} alt={form.shopName} className="h-10 w-10 object-cover" /> : shopInitials}
            </div>
            <div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-4">
          <div className="h-16 w-16 rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden bg-gray-50">
            {form.logoUrl ? <img src={form.logoUrl} alt="logo" className="h-16 w-16 object-cover rounded-lg" /> : <Store className="h-6 w-6 text-gray-300" />}
          </div>
          <div className="space-y-1.5">
            <Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => fileInputRef.current?.click()}>
              <Upload className="h-4 w-4" />ပုံတင်မည်
            </Button>
            {form.logoUrl && (
              <Button type="button" variant="ghost" size="sm" className="gap-2 text-destructive hover:text-destructive" onClick={() => setForm((p) => ({ ...p, logoUrl: "" }))}>
                <X className="h-4 w-4" />ဖယ်ရှားမည်
              </Button>
            )}
      ...
      <div className="space-y-1.5">
        ...
        <Input value={form.shopName} onChange={(e) => setForm((p) => ({ ...p, shopName: e.target.value }))} placeholder="ဥပမာ — မင်းကျော်ဆိုင်" />
      </div>
      <div className="space-y-1.5">
        <Label>ကြော်ငြာစာသား <span className="text-muted-foreground">(ရွေးချယ်နိုင်)</span></Label>
        <Input value={form.shopTagline} onChange={(e) => setForm((p) => ({ ...p, shopTagline: e.target.value }))} placeholder="ဥပမာ — အကောင်းဆုံးဝန်ဆောင်မှု" />
      </div>
      <Button className="gap-2" onClick={handleSave} disabled={saving}>
        <Save className="h-4 w-4" />{saving ? "သိမ်းနေသည်…" : "ဆိုင်အချက်အလက်သိမ်းမည်"}
      </Button>
    </div>
  );
}
...
function CompanyTab() {
  ...
  return (
    <div className="space-y-5 max-w-lg">
      <div className="space-y-4">
        <div className="space-y-1.5"><Label>ဆိုင်/ကုမ္ပဏီအမည်</Label><Input defaultValue="Sales Manager Pro Inc." /></div>
        <div className="space-y-1.5"><Label>ဝဘ်ဆိုက်</Label><Input defaultValue="https://salesmanagerpro.com" /></div>
        <div className="space-y-1.5"><Label>လုပ်ငန်းနယ်ပယ်</Label><Input defaultValue="Retail" /></div>
        <div className="space-y-1.5"><Label>ဆိုင်အရွယ်အစား</Label><Input defaultValue="၁–၁၀ ဝန်ထမ်း" /></div>
        <div className="space-y-1.5"><Label>ငွေကြေး</Label><Input defaultValue="MMK (ကျပ်)" /></div>
        <div className="space-y-1.5"><Label>ဘဏ္ဍာရေးနှစ်စတင်</Label><Input defaultValue="ဇန်နဝါရီ" /></div>
      </div>
...
function NotificationsTab() {
  ...
  const [settings, setSettings] = useState({
    newSale: true, lowStock: true, taskDue: true, weeklyReport: false, emailDigest: true,
  });
  const toggle = (k: keyof typeof settings) => setSettings((p) => ({ ...p, [k]: !p[k] }));

  const items = [
    { key: "newSale"      as const, label: "အရောင်းအသစ်",             desc: "အရောင်းမှတ်တမ်းအသစ် တင်သောအခါ" },
    { key: "lowStock"     as const, label: "ကုန်ပြတ်လုနီးသတိပေးချက်", desc: "ကုန်ပစ္စည်းတစ်ခု သတ်မှတ်ပမာဏအောက် ကျသောအခါ" },
